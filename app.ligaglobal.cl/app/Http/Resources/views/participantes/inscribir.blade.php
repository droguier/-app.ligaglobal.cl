...
<div class="container mt-4">
    <div class="row justify-content-center">
        <div class="col-md-6">
            ...
            <form action="{{ route('participantes.guardar') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <div class="card">
                        <div class="card-header bg-primary text-white border-bottom border-3 rounded-top" style="background: linear-gradient(90deg, #007bff 60%, #00c6ff 100%); box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
                            <strong style="letter-spacing:1px; font-size:1.15rem;">&#x1F4C5; Información del Evento</strong>
                        </div>
                        <div class="card-body">
                            <div class="row mb-2">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-8">{{ strtoupper($evento->nombre ?? '') }}</div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-4"><strong>Fecha:</strong></div>
                                <div class="col-8">
                                    {{ strtoupper(\Carbon\Carbon::parse($evento->fecha_evento ?? '')->translatedFormat('l')) }}, {{ \Carbon\Carbon::parse($evento->fecha_evento ?? '')->translatedFormat('d-m-Y') }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="evento_sub_evento_id" id="evento_sub_evento_id" class="form-select">
                        @if($subeventos->count() > 0)
                            ...
                            @endforeach
                        @else
                            <option value="">Sin sub-eventos a registrar</option>
                        @endif
                    </select>
                </div>
                <div class="mb-3">
